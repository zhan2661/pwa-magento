...
const mediaBases = new Map().set('image-product', process.env.MAGENTO_BACKEND_MEDIA_PATH_PRODUCT || '/media/catalog/product').set('image-category', process.env.MAGENTO_BACKEND_MEDIA_PATH_CATEGORY || '/media/catalog/category');
...
const makeOptimizedUrl = (path, {
  ...
  if (type) {
    ...
    const mediaBase = mediaBases.get(type); // prepend media base if it isn't already part of the pathname
