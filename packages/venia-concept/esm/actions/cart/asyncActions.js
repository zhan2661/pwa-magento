...
export const createGuestCart = () => async function thunk(dispatch, getState) {
  const {
    cart
  } = getState(); // if a guest cart already exists, exit
  ...
  const guestCartId = await retrieveGuestCartId(); // if a guest cart exists in storage, act like we just received it
...
export const addItemToCart = (payload = {}) => {
  ...
  return async function thunk(dispatch, getState) {
    ...
    try {
      ...
      if (!guestCartId) {
        const missingGuestCartError = new Error('Missing required information: guestCartId');
      ...
      const cartItem = await request(`/rest/V1/guest-carts/${guestCartId}/items`, {
        method: 'POST',
        body: JSON.stringify({
      ...
      dispatch(actions.addItem.receive(error)); // check if the guest cart has expired
      ...
      if (noGuestCartId || response && response.status === 404) {
        ...
        await clearGuestCartId(); // then create a new one

        await dispatch(createGuestCart()); // then retry this operation
...
export const updateItemInCart = (payload = {}, targetItemId) => {
  ...
  return async function thunk(dispatch, getState) {
    ...
    try {
      ...
      const cartItem = await request(`/rest/V1/guest-carts/${guestCartId}/items/${targetItemId}`, {
        method: 'PUT',
        body: JSON.stringify({
          cartItem: itemPayload
        })
      ...
      dispatch(actions.updateItem.receive(error)); // check if the guest cart has expired
    ...
    await Promise.all([dispatch(toggleDrawer('cart')), dispatch(getCartDetails({
      forceRefresh: true
...
export const removeItemFromCart = payload => {
  ...
  return async function thunk(dispatch, getState) {
    ...
    try {
      ...
      const cartItem = await request(`/rest/V1/guest-carts/${guestCartId}/items/${item.item_id}`, {
        method: 'DELETE'
      ...
      dispatch(actions.removeItem.receive(error)); // check if the guest cart has expired
...
export const openOptionsDrawer = () => async dispatch => dispatch(actions.openOptionsDrawer());
export const closeOptionsDrawer = () => async dispatch => dispatch(actions.closeOptionsDrawer());
export const getCartDetails = (payload = {}) => {
  ...
  return async function thunk(dispatch, getState) {
    ...
    try {
      const [imageCache, details, paymentMethods, totals] = await Promise.all([retrieveImageCache(), fetchCartPart({
        guestCartId,
        forceRefresh
      }), fetchCartPart({
        guestCartId,
      ...
      const {
        items
      } = details; // for each item in the cart, look up its image in the cache
      ...
      if (imageCache && Array.isArray(items) && items.length) {
        ...
        items.forEach(item => {
          ...
          if (validTotals) {
            const matchingItem = totals.items.find(t => t.item_id === item.item_id);
      ...
      dispatch(actions.getDetails.receive(error)); // check if the guest cart has expired
...
export const toggleCart = () => async function thunk(dispatch, getState) {
  const {
    app,
    cart
  } = getState(); // ensure state slices are present
...
export const removeGuestCart = () => async function thunk(...args) {
...
export async function getGuestCartId(dispatch, getState) {
  const {
    cart
  } = getState(); // reducers may be added asynchronously
