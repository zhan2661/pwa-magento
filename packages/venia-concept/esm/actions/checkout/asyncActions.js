...
import { clearGuestCartId, createGuestCart } from "../cart";
import { getCountries } from "../directory";
import { getOrderInformation } from "../../selectors/cart";
...
export const getShippingMethods = () => {
  return async function thunk(dispatch, getState) {
    ...
    try {
      ...
      const response = await request(`/rest/V1/guest-carts/${guestCartId}/estimate-shipping-methods`, {
        method: 'POST',
        body: JSON.stringify({
          address: {
            country_id: 'US',
...
export const submitOrder = () => async function thunk(dispatch, getState) {
  ...
  try {
    ...
    await request(`/rest/V1/guest-carts/${guestCartId}/shipping-information`, {
      method: 'POST',
      body: JSON.stringify({
        addressInformation: {
          billing_address,
    ...
    const response = await request(`/rest/V1/guest-carts/${guestCartId}/payment-information`, {
      method: 'POST',
      body: JSON.stringify({
    ...
    dispatch(checkoutReceiptActions.setOrderInformation(getOrderInformation(getState(), response))); // Clear out everything we've saved about this cart from local storage.
