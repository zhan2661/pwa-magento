...
function addImgOptMiddleware(app, env = process.env) {
  ...
  debug(`mounting onboard image optimization middleware express-sharp with config %o`, imgOptConfig);
  let cacheMiddleware;
  let sharpMiddleware;
  ...
  try {
    cacheMiddleware = cache(imgOptConfig.cacheExpires, null, {
      ...
      redisClient: imgOptConfig.redis && require('redis').redisClient(imgOptConfig.redis)
    });
  } catch (e) {
    markDepInvalid('apicache', e);
  }
  ...
  if (missingDeps) {
    console.warn(`Cannot add image optimization middleware due to dependencies that are not installed or are not compatible with this environment:
${missingDeps}
...
//# sourceMappingURL=addImgOptMiddleware.js.map