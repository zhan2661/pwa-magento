...
function babelPluginMagentoLayout({
  types: t
}) {
        ...
        exit(programPath, state) {
          ...
          if (!validationRan) {
            ...
            if (typeof prod !== 'boolean') {
              onError(`Expected "prod" to be a boolean, but received "${typeof prod}"`);
            }

            if (typeof config !== 'object') {
              onError(`Expected "config" to be an object, but received "${typeof config}"`);
            }
          ...
          const reactModuleImports = identifiersInScopeFromModule(programPath, /^react$/);
          const {
            reactIdentifier,
          ...
          programPath.traverse({
            CallExpression(path) {
              const isCECall = isCreateElementCall(path, reactIdentifier, createElementIdentifier);
              if (!isCECall) return;
              ...
              if (type !== 'StringLiteral') {
                onWarning('Expected "data-mid" to be a literal string, ' + `but instead found a value of type "${type}"`);
              }

              if (!t.isStringLiteral(element)) {
                onWarning('"data-mid" found on a Composite Component.' + 'Only DOM elements(div/span/etc) can be a Layout Container');
              }

              const operations = config[dataMID]; // No operations were registered for this Container
...
class ContainerOperationsProcessor {
  ...
  getContainerChildIdentifier() {
    const {
      cachedContainerChildIdent
    } = this;
    if (cachedContainerChildIdent) return cachedContainerChildIdent;
    const {
      program
    } = this;
    const containerChildIdent = identifiersInScopeFromModule(program, /^@magento\/peregrine$/).find(({
      bindingName
    }) => bindingName === 'ContainerChild');
  ...
  findContainerChildByName(targetChildID) {
    ...
    const matchingChild = children.find(child => {
      ...
      const isElement = child.isCallExpression() && isCreateElementCall(containerPath, reactIdentifier, createElementIdentifier);
      if (!isElement) return; // Verify the child is a Peregrine ContainerChild
  ...
  buildCreateElementCall(element, props) {
    ...
    const callee = createElementIdentifier ? t.identifier(createElementIdentifier) : t.memberExpression(t.identifier(reactIdentifier), t.identifier('createElement'));
    const elementNode = !t.react.isCompatTag(element) ? t.identifier(element) // Composite Component
    : t.stringLiteral(element); // DOM Element

    return t.callExpression(callee, [elementNode, props || t.nullLiteral()]);
  }
  /**
  ...
  insertAdjacentArgumentsNode(position, callPath, targetArg, exprNode) {
    ...
    const targetIndex = position === 'before' ? targetArgIndex : targetArgIndex + 1;
  ...
  insertElementAdjacentToChild(position) {
    ...
    if (!targetChildPath) {
      onWarning(`Attempted to inject a PWA Studio extension, but specified targetChild was not found\n` + `operation: ${currentOperation.operation}\n` + `targetContainer: ${currentOperation.targetContainer}\n` + `targetChild: ${currentOperation.targetChild}`);
      return;
    }

    const componentIdent = addDefault(targetChildPath, componentPath, {
      nameHint: 'Extension'
    }).name; // TODO: extensionNode needs to be wrapped in a new ContainerChild,
    // and an error boundary

    const extensionNode = this.buildCreateElementCall(componentIdent);
    this.insertAdjacentArgumentsNode(position, containerPath, targetChildPath.node, extensionNode);
  }
  ...
  removeContainer() {
    ...
    if (!isDone) {
      ...
      // between 2 modules
      onWarning(`A remove operation was executed on Container ` + `${containerMID}, but other operations were still ` + 'pending on it. This most commonly indicates' + 'a conflict or ordering issue between modules/extensions');
  ...
  removeChild() {
    ...
    if (!targetChildPath) {
      onWarning(`Attempted to remove a PWA Studio ContainerChild, but could not locate it\n` + 'operation: removeChild\n' + `targetContainer: ${currentOperation.targetContainer}\n` + `targetChild: ${currentOperation.targetChild}\n`);
      return;
    }
...
function getProp(callExprPath, propName) {
  const [, props] = callExprPath.get('arguments'); // Could be a NullLiteral if the element has no props
...
function isCreateElementCall(callExprPath, reactIdent, createElementIdent) {
  const callee = callExprPath.get('callee'); // React.createElement()
...
function identifiersInScopeFromModule(programPath, reModuleSpec) {
  ...
  return targetImportDecls.reduce((acc, importDecl) => {
    for (const spec of importDecl.node.specifiers) {
      acc.push({
        bindingName: spec.type === 'ImportDefaultSpecifier' ? 'default' : spec.imported.name,
...
//# sourceMappingURL=babel-plugin-magento-layout.js.map