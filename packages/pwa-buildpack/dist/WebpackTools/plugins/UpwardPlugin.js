...
const upward = require('@magento/upward-js'); // To be used with `node-fetch` in order to allow self-signed certificates.
...
class UpwardPlugin {
  constructor(devServer, env, upwardPath) {
    ...
    this.upwardPath = upwardPath; // Compose `after` and `before` functions if something else has defined
    // them.
  ...
  apply(compiler) {
    this.compiler = compiler; // If a request has run to the devServer before this method has run,
  ...
  async handleRequest(req, res, next) {
    ...
    await this.middlewarePromise; // When the promise is resolved, `this.middleware` will exist.
    // Replace this function itself.

    this.handleRequest = this.middleware; // And then call it to finish the response.

    this.middleware(req, res, next); // Further requests will go straight to the middleware.
  }

  async createMiddleware() {
    // The compiler is necessary to build the fallback filesystem
    // so UPWARd can use Webpack-generated assets in dev mode.
    const compiler = await this.getCompiler(); // Standard filesystem-and-fetch IO.

    const defaultIO = upward.IOAdapter.default(this.upwardPath); // Use Webpack's in-memory file system for UPWARD file retrieval during
    // development. Allows for hot reloading of server-side configuration.

    const io = {
      async readFile(filepath, enc) {
        const absolutePath = path.resolve(compiler.options.output.path, filepath); // Most likely scenario: UPWARD needs an output asset.
        ...
        try {
          ...
          debug('outputFileSystem %s %s. Trying defaultIO...', filepath, e.message);
      ...
      async networkFetch(path, options) {
        ...
        return fetch(path, options); // Use the https.Agent to allow self-signed certificates.
      }
...
//# sourceMappingURL=UpwardPlugin.js.map