  ...
  apply(compiler) {
    if (this.config.env.mode === 'development') {
      ...
      if (this.config.enableServiceWorkerDebugging && !this.config.injectManifest) {
        ...
        console.warn(`Emitting no ServiceWorker in development mode. To enable development mode for ServiceWorkers, pass \`enableServiceWorkerDebugging: true\` to the ServiceWorkerPlugin configuration.`);
      }
    } else {
...
_defineProperty(ServiceWorkerPlugin, "validateOptions", optionsValidator('ServiceWorkerPlugin', {
  'env.mode': 'string',
