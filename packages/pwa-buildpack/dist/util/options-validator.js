 ...
 */
const lget = require('lodash').get; // A signal object for lodash.get to return, so we can check by reference
...
class BuildpackValidationError extends Error {
  constructor(name, callsite, validationErrors) {
    ...
    this.message = `${name}: Invalid configuration object. ` + `${callsite} was called with a configuration object that has the following problems:${bullet}` + validationErrors.map(([key, requiredType]) => `${key} must be of type ${requiredType}`).join(bullet);
