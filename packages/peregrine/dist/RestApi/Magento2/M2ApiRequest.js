...
var withDefaultHeaders = function withDefaultHeaders(headerAdditions) {
  ...
  if (headerAdditions) {
    if (headerAdditions instanceof Headers) {
      ...
      if (headerAdditions.entries) {
        ...
        try {
          for (var _iterator = headerAdditions[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        ...
        headerAdditions.forEach(function (name, value) {
          headers.append(name, value);
        });
      ...
      var _arr = Object.entries(headerAdditions);
...
/*#__PURE__*/
function () {
  function M2ApiRequest(resourceUrl) {
    var opts = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};

    _classCallCheck(this, M2ApiRequest);

    var storage = new BrowserPersistence();
    var signin_token = storage.getItem('signin_token');
  ...
  _createClass(M2ApiRequest, [{
    ...
    key: "_isMulticastable",
    value: function _isMulticastable() {
    ...
    key: "_fetchMulticast",
    value: function _fetchMulticast() {
      var _this2 = this;
      ...
      var inflightMatch = MulticastCache.match(this); // Is this request meant to override an existing inflight request?

      var rolling = this.isRolling();
      ...
      var promise = this._fetch().catch(function (error) {
        ...
        if (error.name === 'AbortError') {
          var replacedInFlightMatch = MulticastCache.match(_this2);
...
export function request(resourceUrl, opts) {
  var req = new M2ApiRequest(resourceUrl, opts);
  req.run();
  var promise = req.getResponse();
