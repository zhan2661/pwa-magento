...
const cache = new Map();
export const filterProps = (props = {}, blacklist = []) => Object.entries(props).reduce((r, [k, v]) => {
...
const fromRenderProp = (elementType, customProps = []) => {
  ...
  const key = `${elementType}//${uniqueCustomProps.join(',')}`; // only create a new component if not cached
